...
(function ($) {
    ...
    var methods = {
        ...
        init: function (options) {
            ...
            if ($.inArray('backcolor', excludes) == -1  || $.inArray('forecolor', excludes) == -1) {
                colorpicker = true;
            }
            ...
            $.each(groups, function (index, commands) {
                ...
                $.each(commands, function(i, command) {
                    if(jQuery.inArray(command.name, excludes) == -1) { //lets developers exclude buttons (command.name not found in excludes)
